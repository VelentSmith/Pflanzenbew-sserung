...
def page_overview():
    ...
    # Modul-Karten
    cols = st.columns(2)
    for idx, (m_id, mod) in enumerate(list(backend.Modules.items())): # list() für sichere Iteration
        with cols[idx % 2]:
            with st.container(border=True):
                ...
                c_b.metric("🌿 Belegte Plätze", f"{len(mod.pots)} / 4")
    ...
    for pos, pot in mod.pots.items():
        with st.container(border=True):
            ...
            with cols[1]:
                with st.expander("⚙️ Einstellungen"):
                    ...
                    if st.button("Speichern", key=f"sv_{pos}", type="primary"):
                        calc_minutes = round(get_time_backend_minutes(new_time_val, t_unit_sel))
                        calc_ml = round(get_water_backend_ml(new_amount_val, w_unit_sel))
            ...
            with cols[2]:
                ...
                if st.button("🗑️ Pflanze löschen", key=f"del_{pos}", type="primary"):
                    mod.DeletePot(pos)
                    log_event(m_id, f"Pflanze {pos} gelöscht", "CONFIG")
                    st.rerun()
